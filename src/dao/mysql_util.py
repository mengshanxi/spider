...
class MysqldbHelper(object):
    ...
    def __init__(self, host=host, username=username, password=password, port=3306, database=database):
        self.host = host
    ...
    def selectOne(self, tablename, cond_dict='', fields='*'):
        """
        查询数据
            args：
                tablename  ：表名字
    ...
    def delete(self, tablename, cond_dict):
        ...
        if cond_dict != '':
            for k, v in cond_dict.items():
                if isinstance(v, str):
                    v = "\'" + v + "\'"
                consql = consql + tablename + "." + k + '=' + str(v) + ' and '
        consql = consql + ' 1=1 '
